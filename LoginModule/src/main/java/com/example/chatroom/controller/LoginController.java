...
public class LoginController {
    ...
    @GetMapping("/sendCode")
    @Operation(summary = "获取验证码",responses = {@ApiResponse(description = "返回验证码的图片")})

    public Result<String> sendCode(String account){//可以为手机号也可以是邮箱，注册时使用，或者找回密码时使用
        log.info("{}获取验证码",account);
        String code;
        try {
             code= loginService.sendCode(account);
    ...
    public Result findPassword(@RequestBody RegisterDTO registerDTO){//找回密码
        try{
            loginService.findPassword(registerDTO.getUsername(),registerDTO.getPassword(), registerDTO.getCode());
            return Result.success();
        }catch(CodeNotExistException e){
            return Result.fail("验证码不存在");
        }catch(CodeNotPassException e){
    ...
    @PostMapping
    @Operation(summary = "登录",responses = {@ApiResponse(description = "生成token")})
